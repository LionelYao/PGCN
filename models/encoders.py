...
import layers.product_layers as product_layers
...
class HNN(Encoder):
    ...
    def encode(self, x, adj):
        ...
        #print("dims", self.manifold.time_dim,self.manifold.dim, x.shape)
...
class PNN(Encoder):
    """
    Product Manifold Neural Networks.
    """
    def __init__(self, c, args):
        super(PNN, self).__init__(c)
        self.manifold = ProductManifold([getattr(manifolds, name)(space_dim=args.space_dim_list[i], time_dim=args.time_dim_list[i]) for i,name in enumerate(args.manifold_list)])
        assert args.num_layers > 1
        dims, acts, self.curvatures = product_layers.get_dim_act_curv(args)
        self.dim_split = dims[0]
        #print()
        self.curvatures.append(self.c)
        pnn_layers = []
        for i in range(len(dims) - 1):
            ...
            act = acts[i]
            pnn_layers.append(
                    product_layers.PNNLayer(
                            self.manifold, in_dim, out_dim, self.c, args.dropout, act, args.bias)
            )
        self.layers = nn.Sequential(*pnn_layers)
        self.encode_graph = False

    def encode(self, x, adj):
        time_dim = self.manifold.time_dims
        print(self.manifold.dims, self.manifold.time_dims)
        #for i in range(len(self.dim_split)):
        #    if self.manifold.time_dims[i]<self.manifold.dims[i]:
        #        time_dim[i] = self.manifold.time_dims[i]
        #else:
        #    time_dim[i] = int((self.manifold.time_dims[i]/self.manifold.dims[i])*self.dim_split[i])
        #print(x.shape)
        x_split = torch.split(x, self.dim_split, dim=-1)
        #print(x_split)
        #print([k.shape for k in x])
        curvatures = [self.c]*len(time_dim)
        x_tan = self.manifold.proj_tan0(x_split, curvatures, time_dim=time_dim)
        x_hyp = self.manifold.expmap0(x_tan, curvatures, time_dim=time_dim)
        x_hyp = self.manifold.proj(x_hyp, curvatures,time_dim=time_dim)
        x = torch.cat(x_hyp, dim = -1)
        #print("dim after PNN encoder", x.shape)
        #print([x.shape for x in x_hyp])
        return super(PNN, self).encode(x, adj)

class PGCN(Encoder):
    """
    Product Manifold-GCN.
    """
    def __init__(self, c, args):
    ...
    def encode(self, x, adj):
        ...
        #else:
        x = torch.cat(x_hyp, dim = -1)
        output = super(PGCN, self).encode(x, adj)
        return output
