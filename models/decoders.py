...
from layers.product_layers import ProGraphConv
...
class ProLinearDecoder(Decoder):
    """
    MLP Decoder for Product manifold node classification models.
    """

    def __init__(self, manifold, c, args):
        super(ProLinearDecoder, self).__init__(c)
        ...
        self.input_dim = sum(args.dim_list)
        self.input_dims = args.dim_list
        self.output_dim = args.n_classes
        self.bias = args.bias
        self.cls = Linear(self.input_dim, self.output_dim, args.dropout , lambda x: x, self.bias)
        self.decode_adj = False
    ...
    def decode(self, x, adj):
        #print(x.shape)
        #print(self.manifold.manifolds)
        x_splits = torch.split(x, self.input_dims, dim=-1)
        h = self.manifold.proj_tan0(self.manifold.logmap0(x_splits, [self.c]*len(self.input_dims)), [self.c]*len(self.input_dims))
        #print(h)
        h = torch.cat(h, dim=-1)
        return super(ProLinearDecoder, self).decode(h, adj)
...
class PGCNDecoder(Decoder):#use
    ...
    def __init__(self, manifold, c, args):
        ...
        act = lambda x: x
        # manifold, in_features, out_features, c_in, c_out, act, use_bias, use_att, local_agg
        self.cls = ProGraphConv(manifold, args.dim_list, args.n_classes, self.c, self.c, args.dropout, act, args.bias ,args.local_agg)
        self.decode_adj = True
...
model2decoder = {
    ...
    'PNN': ProLinearDecoder,
    'PGCN':ProLinearDecoder
}
