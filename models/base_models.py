...
class BaseModel(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.manifold_name == 'ProductManifold': #need to parse space_dim,time_dim,feat_dim,manifold to lists
            ...
            #print(args.feat_dim)
            for i,m in enumerate(args.manifold_list):
                if m in ['Hyperboloid', 'PoincareBall']:
                    self.c = nn.Parameter(torch.Tensor([1.]))
                else:
                    self.c = nn.Parameter(torch.Tensor([-1.]))
                if m in ['Hyperboloid', 'PseudoHyperboloid']:
                    args.feat_dim = args.feat_dim + 1
            #print(args.feat_dim)
            self.init_dims = almost_equal_split(args.feat_dim, len(args.dim_list))
        else:
    ...
    def encode(self, x, adj):
        if self.manifold.name in ['Hyperboloid','PseudoHyperboloid']:
            ...
            #print(x.shape)
        #if self.manifold
        if self.manifold.name == 'ProductManifold':
            x_split = list(torch.split(x, [dim - 1 for dim in self.init_dims], dim=-1))
            for i in range(len(x_split)):
                o = torch.zeros_like(x_split[i])
                o[:, 0:1] = 1.0
                x_split[i] = torch.cat([o[:, 0:1], x_split[i]], dim=1)
                #print(x_split[i])
                #print(x.shape)
            #print([k.shape for k in x_split])
            x = torch.cat(x_split, dim = -1)
            #assert x.dim() == 2
            #print(tuple(x_split))
            #print("dim after first split", x.shape)
            h = self.encoder.encode(x, adj)
        #print("a")
